<?php
	...
	$paperObj -> s_Search( $Pid );
	$paperObj -> getOneRecord();
	$pname = $paperObj -> getPname();
	//查出所有活动下属学科ID
	...
	$questionObj -> a_Search( 0 ,0 ,  $Pid );
	$count = $questionObj -> getRecordCount();
	$list = "";
	$questionObj -> moveFirst();
	for( $i = 1 ; $i <= $count ; $i ++ ){
		$questionObj -> getOneRecord();
		$list .= $i < $count ? $questionObj -> getQid() ."," : $questionObj -> getQid();
...
<html>
	...
	<body>
		<div style="margin: 15px;">
			...
			<form class="layui-form" method="post" action = "../../service/paper/setQuestion.php" >
				<fieldset  class="layui-elem-field layui-field-title" style="margin-top: 20px;">
					<legend>编辑下属题目</legend>
				</fieldset>
				<input hidden type="text" id = "Pid" name="Pid" value = "<?php echo $Pid ; ?>">
				<div class="layui-form-item layui-form-text">
					<label class="layui-form-label">试卷<?php echo "$Pid :  " ?> </label>
					<label class="layui-form-label"><?php echo urldecode( "$pname") ?>  </label>
				...
				<div class="layui-form-item layui-form-text">
					...
					<div   class="layui-input-block"  >
						...
						<font color="#FF0000">多个题目ID用英文逗号隔开</font> 
						</input>
					</div>
				</div>
